import { Outlet } from "react-router-dom"
import { NavItem } from "./NavItem"
import { BookImage, LayoutDashboard, SquarePlus } from "lucide-react"
import { Show, SignInButton, SignOutButton, SignUpButton, UserButton } from "@clerk/react"
import { Button } from "./ui/button"
import { Card } from "./ui/card"

export const AppLayout = () => {
    return (
        <div className="flex min-h-screen bg-auto">
            <div className="w-64 fixed h-screen overflow-hidden bg-sidebar">
                <h1 className="pt-4 text-center text-2xl font-bold bg-clip-text from-blue-400 to-purple-400 text-transparent bg-linear-to-r">Oskado</h1>
                <div className="flex flex-col p-1 m-2 gap-1">
                    <NavItem to="/dashboard" label="Dashboard" icon={LayoutDashboard} end />
                    <NavItem to="/catalog" label="Catalog" icon={BookImage} actionButton actionIcon={SquarePlus} actionTooltip="Add New Item" actionTo="/addItem" />
                    {/* <NavItem to="/addItem" label="Add Item" icon={SquarePlus} /> */}
                    {/* <NavItem to="/transactions" label="Transactions" icon={BanknoteArrowUp} />
                    <NavItem to="/history" label="History" icon={Sheet} />
                    <NavItem to="/analytics" label="Analytics" icon={ChartNoAxesCombinedIcon} /> */}
                </div>
            </div>
            <div className="ml-64 flex-1 flex flex-col h-screen bg-muted">
                <div className="sticky flex flex-row justify-end gap-4 items-center top-0 h-12 z-10 bg-sidebar p-3">
                    <Show when="signed-out">
                        <SignInButton>
                            <Button>Sign In</Button>
                        </SignInButton>
                        <SignUpButton>
                            <Button variant="secondary">Sign Up</Button>
                        </SignUpButton>
                    </Show>
                    <Show when="signed-in">
                        <SignOutButton>
                            <Button variant="secondary">Sign Out</Button>
                        </SignOutButton>
                        <UserButton />
                    </Show>
                </div>
                <Card className="m-4 py-0 overflow-y-auto h-[calc(100vh-3rem-2rem)]">
                    <Outlet />
                </Card>
            </div>
            
        </div>
    )
}