...
export const addItemSchema = z.object({
    ...
    categoryId: z.number().min(1, "Please Select a Product Category"),
    subCategoryId: z.number().optional(),
    locationId: z.number().min(1, "Please Select a Purchased Location"),
    purchasePrice: z.number().min(.01, "Please add a purchase price for the item."),
    datePurchased: z.date().refine(d => d <= new Date(), {error: "Purchase Date must be today or earlier"}).optional(),
